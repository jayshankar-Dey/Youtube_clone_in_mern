...
import Liked_Vedios_Card from "../components/Liked_Vedios_Card"
...
const Liked_Vedios = () => {
  ...
  // eslint-disable-next-line no-unused-vars
    ...
    useEffect(()=>{
      const fetchVedios=async()=>{
        try {
          const res=await axios.get(`${base}/getLike_vedios`,{
            headers:{
              Authorization:`Bearer ${token}`
            }
          })
          setVedios(res.data.LikeVedio)
  ...
  const deleteLike=async(id)=>{
    try {
      const res=await axios.delete(`${base}/delete/Like/${id}`,{
        headers:{
            Authorization:`Bearer ${token}`
        }
      })
  ...
  return (
    <div className="h-screen flex flex-col w-full bg-zinc-950 overflow-scroll">
     ...
     {vedios?.length>0&& <div className="lg:w-96 lg:border rounded-tl-xl rounded-tr-xl border-zinc-800 bg-[#e3e3e3a2]">

<div className="w-44 md:w-52  min-h-52 bg-zinc-900   rounded overflow-hidden mx-auto mt-2">
          {
            vedios[0]?.isimage?<img className="h-full w-full object-cover" src={vedios[0]?.image?.url}></img>:<Link to={`/single/vedio/${vedios[0]?._id}`}>
            {
    ...
    <h1 className="text-2xl text-white font-bold mt-4 px-2">Liked Videos</h1>
    {vedios[0]?.isimage?<h1 className="px-2 text-white font-semibold ">{vedios[0]?.text}</h1>:<h1 className="px-2 text-white font-semibold ">{vedios[0]?.name}</h1>}
   ...
   <span className="text-sm"> {vedios?.length} videos </span>
      ...
      <div className=" flex flex-col gap-y-2 md:p-3  p-1 lg:w-[50rem] overflow-scroll">
        ...
        {
            vedios?.map((data,i)=>(
                <Liked_Vedios_Card key={i} vedio={data} deleteLike={deleteLike}/>
            ))
        }
       </div>
      </div>
